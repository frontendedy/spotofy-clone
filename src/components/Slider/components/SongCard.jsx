...
const SongCard = ({ data }) => {
  ...
  // console.log(data.songs && data.songs.length, 'data')
  return (
    <div className='group transition inline-block duration-400 ease-in-out w-[210px] bg-light-grey rounded-[8px] p-3 hover:bg-grey cursor-pointer relative'>
      {
        data.songs && <div class="opacity-0 w-28 bg-grey text-white text-center text-xs rounded py-2 absolute z-[999] group-hover:opacity-100 px-4 pointer-events-none top-[-44px] right-0 left-0 m-auto">
          {data.songs.length} Songs
          <svg class="absolute text-white h-2 w-full left-0 top-full" x="0px" y="0px" viewBox="0 0 255 255">
            <polygon className="fill-current" points="0,0 127.5,127.5 255,0" fill="#444444" />
          </svg>
        </div>
      }
